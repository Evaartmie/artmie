...
async function generateAllReturnsCSV(where: any): Promise<string> {
  ...
  const header = csvRow([
    ...
    "Refundovana suma", "Poznamky admin",
    "Pocet fotiek", "Schvalene", "Zamietnute", "Prijate", "Refundovane", "Uzavrete",
  ]);
...
async function generateReasonsCSV(where: any): Promise<string> {
  ...
  const reasonMap = new Map<string, {
    reason: string; total: number; shops: Record<string, number>;
    totalValue: number;
  }>();
  ...
  for (const r of returns) {
    for (const li of r.lineItems) {
      ...
      const existing = reasonMap.get(reason) || {
        reason, total: 0, shops: {}, totalValue: 0,
      };
  ...
  const header = csvRow([
    "Dovod", "Celkom", "Celkova hodnota", ...shopList,
  ]);
...
async function generateCustomersCSV(where: any): Promise<string> {
  ...
  const header = csvRow([
    ...
    "Cakajuce", "Schvalene", "Zamietnute", "Refundovane",
  ]);
